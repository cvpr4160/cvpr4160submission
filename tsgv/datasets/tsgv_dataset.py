    ...
    def __generate_sentence_features__(self):
        ...
        if distributed.get_rank() == 0: # h5py dose not support multiple I/O
            ...
            with h5py.File(self.sentence_feat_file, 'a') as File:
                for anno_idx in tqdm(range(self.anno_infos.shape[0]), desc='Generating sentence features...', mininterval=10.):
                    ...
                    if sentence not in File:
                        ...
                        with torch.no_grad():
                            ...
                            # [token_num, 768]
                            File.create_dataset(sentence, data=sentence_emb)
